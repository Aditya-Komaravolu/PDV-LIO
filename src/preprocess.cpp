...
void Preprocess::process(const livox_ros_driver::CustomMsg::ConstPtr &msg, PointCloudXYZINormal::Ptr &pcl_out) {
    avia_handler(msg);
    *pcl_out = pl_surf;
}

void Preprocess::process(const sensor_msgs::PointCloud2::ConstPtr &msg, PointCloudXYZINormal::Ptr &pcl_out) {
    switch (lidar_type) {
        case OUST64:
            oust64_handler(msg);
            break;
...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg) {
    ...
    if (feature_enabled) {
        ...
        for (int j = 0; j < N_SCANS; j++) {
            ...
            pcl::PointCloud<pcl::PointXYZINormal> &pl = pl_buff[j];
            plsize = pl.size();
        ...
        for (uint i = 1; i < plsize; i++) {
            if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00)) {
                ...
                if (valid_num % point_filter_num == 0) {
                    ...
                    // 与上一个添加进去的点比较
                    if (((abs(pl_full[i].x) > 1e-7) || (abs(pl_full[i].y) > 1e-7) || (abs(pl_full[i].z) > 1e-7)) && (pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z > (blind * blind))) {
...
void Preprocess::oust64_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    if (feature_enabled) {
        ...
        for (int j = 0; j < N_SCANS; j++) {
            PointCloudXYZINormal &pl = pl_buff[j];
...
void Preprocess::give_feature(pcl::PointCloud<pcl::PointXYZINormal> &pl, vector<orgtype> &types) {
    int plsize = pl.size();
    int plsize2;
    if (plsize == 0) {
...
void Preprocess::pub_func(PointCloudXYZINormal &pl, const ros::Time &ct) {
    pl.height = 1;
    pl.width = pl.size();
...
int Preprocess::plane_judge(const PointCloudXYZINormal &pl, vector<orgtype> &types, uint i_cur, uint &i_nex, Eigen::Vector3d &curr_direct) {
...
bool Preprocess::edge_jump_judge(const PointCloudXYZINormal &pl, vector<orgtype> &types, uint i, Surround nor_dir) {
    if (nor_dir == 0) {
